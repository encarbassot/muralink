...
function buildDecorations(view: EditorView, cfg: DecoConfig): DecorationSet {
  const { state } = view
  // Read-only editors have no meaningful cursor: never reveal raw syntax
  // (links/headings must render pretty when the surface isn't editable).
  const cursors = state.facet(EditorView.editable)
    ? buildCursorSet(view)
    : new Set<number>()
    ...
    enter(ref: SyntaxNodeRef) {
      ...
      switch (name) {
        ...
        case 'OrderedList': {
          ...
          // Skip the button when the last item is empty (just its marker): its
          // widget would land on the caret's own empty line and CodeMirror
          // mis-measures the caret onto the button row. Right after Enter you're
          // already on a fresh item, so the button is redundant there anyway.
          if (/^\s*([-*+]|\d+\.)\s*$/.test(lastLine.text)) break
          ranges.push({
            from: lastLine.to,
            to:   lastLine.to,
            // NOTE: no `block: true` — CodeMirror forbids block decorations
            // from view plugins (hard error on ≤6.38, corrupted tile DOM and a
            // dead updateListener on ≥6.39). Inline widget at end of line.
  ...
  return ViewPlugin.fromClass(
    class {
      ...
      private updateSelectedLines(view: EditorView) {
        ...
        // No selected-line styling on read-only surfaces.
        const ranges = view.state.facet(EditorView.editable) ? view.state.selection.ranges : []
