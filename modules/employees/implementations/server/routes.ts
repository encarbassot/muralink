...
export function createEmployeesRouter(db: Database): Router {
  ...
  // Routes are relative — this router is mounted at /api/employees
  // (platforms/server/src/index.ts), so '/' here is already /api/employees.
  //
  // Shifts' static routes ('/shifts', '/shifts/:id') are registered BEFORE
  // employees' '/:id' — Express matches in registration order, and '/:id' is a
  // single-segment wildcard that would otherwise shadow GET/POST '/shifts'
  // (matching it as an employee lookup with id="shifts").
